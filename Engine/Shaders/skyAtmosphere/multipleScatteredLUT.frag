    ...
    for (int i = 0; i < sqrtSamples; i++) {
        for (int j = 0; j < sqrtSamples; j++) {
            ...
            for (float stepI = 0.0; stepI < mulScattSteps; stepI += 1.0) {
                ...
                vec3 sunTransmittance = getValFromTLUT(iChannel0, iChannelResolution.xy, newPos, sunDir);
            ...
            if (groundDist > 0.0) {
                ...
                if (dot(pos, sunDir) > 0.0) {
                    ...
                    lum += transmittance * groundAlbedo * getValFromTLUT(iChannel0, iChannelResolution.xy, hitPos, sunDir);
                }
            }
